...
exports.createProduct = async (req, res) => {
    try {
        ...
        const validatedVariants = variants.map((variant, index) => {
            ...
            // Validación de imágenes (pero sin bloquear el producto si están incorrectas)
            let validatedImages = [];
            if (variant.images && Array.isArray(variant.images)) {
                validatedImages = variant.images.filter(img => typeof img === 'string' && img.startsWith('http'));
            }

            return {
                ...variant,
                images: validatedImages, // Si no hay imágenes válidas, se guarda vacío
